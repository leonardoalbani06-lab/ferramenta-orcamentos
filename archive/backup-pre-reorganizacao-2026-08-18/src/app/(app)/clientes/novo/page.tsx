    ...
    <main className="mx-auto max-w-2xl p-8">
      ...
      <form action={criarCliente} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div className="mt-2 flex gap-3 sm:col-span-2">
          <button
            type="submit"
            className="rounded-lg bg-brand-olive px-4 py-2 font-medium text-white transition hover:bg-brand-oliveDark"
          >
            Salvar
          </button>
          <a
            href="/clientes"
            className="rounded-lg px-4 py-2 text-gray-600 transition hover:text-brand-olive hover:underline"
          >
            Cancelar
          </a>
        </div>
  ...
  return (
    <div className={`flex flex-col gap-1 ${className}`}>
      ...
      <input
        ...
        required={required}
        maxLength={maxLength}
        className="rounded-lg border border-brand-olive/20 px-3 py-2 outline-none transition focus:border-brand-olive focus:ring-2 focus:ring-brand-olive/20"
      />
    </div>
  );
}
